...
sheet = wb.sheet_by_name("GetHolidays")
sheet2 = wb.sheet_by_name("HolidaysList")
...
for x in range(0, elementiterator, 2):
    ...
    GetIncResp = driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div[3]/div[2]/div[2]/div["+str(z)+"]/div") \
            .get_attribute('textContent')
    print("Response :" + GetIncResp)
    # print("Test Case Get Holidays for row " + str(index))
    for i in range(0,sheet2.nrows):
        if GetIncResp.find(sheet2.cell_value(i, 1)) != -1:
            print("Holiday Listed:"+sheet2.cell_value(i, 1))
        else:
            print("Holiday not in list: "+sheet2.cell_value(i, 1))

    index = index+1
    if GetIncResp.find("holidays") != -1:
